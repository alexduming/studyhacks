...
**注意：** 由于 drizzle-kit 版本兼容性问题，不推荐使用 `npx drizzle-kit push:pg` 命令。
...
1. **数据库 Schema**
   - `src/config/db/schema.ts` - 添加 invitation 表定义

2. **注册流程**
   - `src/app/api/auth/register-with-email/route.ts` - 集成邀请码验证和奖励发放

3. **注册表单**
   - `src/shared/components/auth/register-complete-page.tsx` - 添加邀请码输入框
...
3. 填写邮箱并接收验证邮件
4. 点击验证链接后，在注册表单中输入邀请码（如果URL中有邀请码会自动填充）
5. 完成注册
